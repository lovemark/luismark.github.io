...
var luisSnap = {
  ...
  // Sprite list for this stage. (primitive in first version, fixed in 1.4.0)
  sprites: {},
  ...
  init: function(vars) {
    ...
    instructions[206] = this.mod;
    instructions[207] = this.round;
    ...
    instructions[230] = this.addList;
    instructions[231] = this.delete;
    instructions[232] = this.insert;
    instructions[233] = this.setListItem;
    instructions[234] = this.getListItem;
    instructions[235] = this.length;
    instructions[236] = this.contains;
    instructions[242] = this.move;
  ...
  current: function(x) {
    var map = ["getFullYear", "getMonth", "getDate", "getDay", "getHours", "getMinutes", "getSeconds", "getTime"]; // added features with v1.4.0
  ...
  // Gets the remainder of the division of two numbers. (v1.4.0)
  mod: function(a, b) {
    return a % b;
  },
  
  // Rounds a number. (v1.4.0)
  round: function(a) {
    return Math.round(a);
  },
  
  // Gets the specified item of the specified list. (v1.4.0)
  getListItem: function(l, i) {
    return this.variables[l][i];
  },
  
  // Adds a item to the specified list. (v1.4.0)
  addList: function(l, i) {
    this.variables[l].push(i);
  },
  
  // Renders all sprites. (v1.4.0)
  render: function() {
    jQuery.each(this.sprites, function(i, s) {
      var ctx = this.canvas.getContext("2d");
      if (s.shown) {
        var img = new Image();
        img.src = s.costumes[s.costumeNum - 1];
        var w = img.width, h = img.height;
        ctx.drawImage(0, 0, w, h, 240 + s.x - (w / 2), 240 - s.y - (h / 2), w, h);
      }
      if (s.clones) {
        jQuery.each(s.clones, function(j, c) { 
          if (s.shown) {
            var img2 = new Image();
            img2.src = c.costumes[c.costumeNum - 1];
            var w2 = img2.width, h2 = img2.height;
            ctx.drawImage(0, 0, w2, h2, 240 + c.x - (w2 / 2), 240 - c.y - (h2 / 2), w2, h2);
          }
        });
      }
    });
  ...
  // Gets the length of the specified list. (v1.4.0)
  length: function(l) {
    return this.variables[l].length;
  },
  
  // Deletes the specified item from the specified list. (v1.4.0)
  delete: function(l, i) {
    this.variables[l].splice(i - 1, 1);
  },
  
  // Inserts a element at the specified index in the specified list. (v1.4.0)
  insert: function(l, i, s) {
    this.variables[l].splice(i, 0, s);
  },
  
  // Sets the specified item of the specified list to the specified value. (v1.4.0)
  setLisItem: function(l, i, s) {
    this.variables[l][i] = s;
  },
  
  // Checks if the specified list contains the specified value. (v1.4.0)
  contains: function(l, s) {
    return l.indexOf(s) != -1;
  },
  
  // Moves a variable monitor to the specified location. (v1.4.0, request by @Masquerola)
  moveVar: function(v, x, y) {
    this.varmonitors[v].x = x;
    this.varmonitors[v].y = y;
  }
}.checkForJQuery();
...
$(document).ready(function() {
  ...
  window.luisSnap.keyPressedInit();
  window.luisSnap.mouseDownInit();
});
